...
import { RacerPosition, GeoPoint } from '../types';
...
const useRaceState = (modelId: string, parameters?: any) => {
  ...
  const [racerPosition, setRacerPosition] = useState<RacerPosition>({
    x: 0,
    y: 0,
    z: 0,
    rotation: 0,
  ...
  const [opponentPosition, setOpponentPosition] = useState<RacerPosition>({
    x: 0,
    y: 0,
    z: 0,
    rotation: 0,
  ...
  const interpolatePosition = (progress: number, route: number[][] = selectedRoute): RacerPosition => {
