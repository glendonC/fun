...
import { Zap, AlertTriangle, Brain, Cpu, CornerUpRight, Shuffle } from 'lucide-react';
...
const RaceEffects: React.FC<RaceEffectsProps> = ({ 
  ...
  // State for obstruction effects
  const [showObstructionEffect, setShowObstructionEffect] = useState(false);
  ...
  useEffect(() => {
    const handleObstruction = (e: CustomEvent) => {
      ...
      setShowObstructionEffect(true);
      
      // Add visual effects based on obstruction type
      if (type === 'blocked') {
        setShowWobble(true);
      } else if (type === 'rerouting') {
        setShowProcessing(true);
      } else if (type === 'evading') {
        setShowGlitch(true);
      }
      
      // Hide obstruction effect after a delay
      const timer = setTimeout(() => {
        setShowObstructionEffect(false);
    ...
    <>
      ...
      <div className="absolute top-1/4 left-1/2 transform -translate-x-1/2 pointer-events-none z-20">
        <AnimatePresence>
          ...
          {showObstructionEffect && (
            <motion.div 
              className={`bg-${
                obstructionType === 'blocked' ? 'red' : 
                obstructionType === 'rerouting' ? 'yellow' : 
                obstructionType === 'evading' ? 'blue' : primaryColor
              }-500 text-white px-6 py-3 rounded-full shadow-lg flex items-center`}
              initial={{ opacity: 0, y: 20, scale: 0.8 }}
              animate={{ 
                opacity: 1, 
                y: 0, 
            ...
            >
              {obstructionType === 'blocked' ? (
                ...
                <CornerUpRight className="h-6 w-6 mr-2" />
              ) : (
              ...
              <span className="text-xl font-bold">
                {obstructionType === 'blocked' ? (
                  isRat ? `🛑 Path blocked by ${opponentName}!` : `⚠️ ${opponentName} blocking your path!`
                ) : obstructionType === 'rerouting' ? (
                  isRat ? `🔄 Rerouting to avoid ${opponentName}...` : `↪️ Changing course!`
                ) : (
                  isRat ? `⚡ Evasive maneuvers!` : `💨 Quick dodge!`
                )}
              </span>
            </motion.div>
          )}
          ...
          <div className="space-y-4">
            ...
            {showObstructionEffect && (
              <div>
                <div className="flex justify-between text-sm mb-1">
                  <span className="text-gray-300">
                    {obstructionType === 'blocked' ? 'Path Blockage' : 
                     obstructionType === 'rerouting' ? 'Rerouting' : 
                     'Evasion Maneuver'}
                  </span>
                  <motion.div 
