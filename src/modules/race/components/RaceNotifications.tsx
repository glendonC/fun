...
import { Flag, Clock, Award, Medal, Building, AlertTriangle, CornerUpRight, Shuffle } from 'lucide-react';
...
const RaceNotifications: React.FC<RaceNotificationsProps> = ({ 
  ...
  const [notifications, setNotifications] = useState<Array<{
    id: string;
    type: 'checkpoint' | 'start' | 'complete' | 'reset' | 'nasdaq' | 'lead' | 'behind' | 'obstruction';
    message: string;
  ...
  useEffect(() => {
    ...
    const handleObstruction = (e: CustomEvent) => {
      ...
      switch (type) {
        case 'blocked':
          message = isRat 
            ? `Path blocked by ${opponentName}! Recalculating...` 
            : `${opponentName} blocking your flight path!`;
          icon = <AlertTriangle className="h-5 w-5" />;
          color = 'red';
          break;
        case 'rerouting':
          message = isRat 
            ? `Rerouting to avoid ${opponentName}'s trajectory...` 
            : `Changing course to avoid ${opponentName}'s path!`;
          icon = <CornerUpRight className="h-5 w-5" />;
          color = 'yellow';
          break;
        case 'evading':
          message = isRat 
            ? `Evasive maneuvers! ${opponentName} crossing path!` 
            : `Quick dodge! ${opponentName} cutting across!`;
  ...
  }, [primaryColor, opponentColor, isRat, opponentName]);
